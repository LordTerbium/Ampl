...
/**
 * The class contains variables that define the position of the Launcher and Minecraft-Window if you have more monitors. It also auto-saves the changing of the position.
 *
 * @author Max
 */
public class Location
{
    ...
    private transient GraphicsEnvironment GEnv     = GraphicsEnvironment.getLocalGraphicsEnvironment();
    private transient GraphicsDevice[]    GDev     = GEnv.getScreenDevices();

    /**
     * Returns the offset on the currently used monitor. This are not the positional coordinates from the upper left corner of the main monitor.
     *
     * @return the offSet
     */
    ...
    /**
     * Sets the location on the screen, which is currently used. So for example, if we had a monitor on the right of our standard monitor and this device had the id = 1, we could set the offset starting by the begin of the second monitor. This is more intuitive because we are seeing two separated screens.
     *
    ...
    /**
     * This method refreshes the absolute coordinates of the frame by taking the provided device-id and adding the chosen offset from the user. If the device is not found, it will default to the main monitor. If even this doesn't work, the program stops.
     */
    public void setScreen ()
    {
        ...
        } else if ( GDev.length > 0 )
        {
            ...
            logger.error( "Couldn't find the requested <bold>monitor</bold>.<br>I'm going to start the Launcher in the " + "default monitor." );
        } else
        {
